...
//	bDeviceIDRequested(false),
	...
	bool Camera::setup() {
        ...
        if (deviceList.size() == 0) {
            ofLogWarning("Camera: no cameras found")<<deviceID;
			return false;
		}
...
//            pixels.set
//            pixels.setFromExternalPixels(cameraFrame.ImageBuffer, 648, 488, 3); 
...
//            ofLog()<<"cameraFrame.ImageBuffer = pixels.getPixels()";
			//	pixels = new unsigned char[frameSize];
            cameraFrame.ImageBuffer = pixels.getData(); //this is where ofPixels get linked/referenced to ImageBuffer
