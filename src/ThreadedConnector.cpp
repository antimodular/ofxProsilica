...
namespace ofxProsilica {
	...
	void ThreadedConnector::update() {
        ofLog()<<"ThreadedConnector::update()";
		if(lock()) {
			TbIsFrameNew = Tgrabber.isFrameNew();
			if(TbIsFrameNew) {
                ofLog()<<"TbIsFrameNew "<<TbIsFrameNew;
				// copy pixels
//				ofPixels pRef = Tgrabber.getPixels();
//                Tpixels.setFromPixels(Tgrabber.getPixels()); //, Tgrabber.getWidth(), Tgrabber.getHeight(), Tgrabber.getPixels().getNumChannels());
                Tpixels = Tgrabber.getPixels();
...
//        if (isMainThread()) {
            ...
            if (TbIsFrameNew){
                ...
                ofLog()<<"texture.getTextureData "<<texture.getTextureData().glInternalFormat;
                
                if (texture.getWidth() != Twidth || texture.getHeight() != Theight || texture.getTextureData().glInternalFormat != GL_LUMINANCE) {
                    texture.clear();
...
//                texture.loadData(Tpixels.getPixels(), Twidth, Theight, GL_LUMINANCE);
                ...
                texture.loadData(Tpixels.getData(), Twidth, Theight, GL_LUMINANCE);
            }
//        }
...
//	}
	...
	ofPixels  ThreadedConnector::getPixels() {
		return Tpixels;
	}
